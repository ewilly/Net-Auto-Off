...
public class Receiver extends BroadcastReceiver {
     ...
     *            the context
     * @param id
     *            TIMER_SCREEN_OFF_WIFI or TIMER_NO_NETWORK_WIFI
     * @param time
     *            in min
     */
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		} else if (ScreenOffDetector.SCREEN_ON_ACTION.equals(action)) {
     ...
     *            the context
     * @param state
     *            on to turn WiFi on, off to turn it off
     */
        ...
        if (state.equals(ON)) {
            try {
                if (isAirplaneModeOn(context)) {
                    if (LOG) Log.d(LOG_TAG, "not turning wifi on because device is in airplane mode");
                    return;
                } else {
                    try {
     ...
     *            the context
     * @param state
     *            on to turn Data on, off to turn it off, mms to turn it to MMS only
     */
